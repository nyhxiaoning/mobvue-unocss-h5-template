...
export default class WebRecorder {
  ...
  start() {
    ...
    const getAudioFail = (err) => {
      new this.OnError(err)
      this.stop()
    }

    if (this.audioContext) {
      new this.OnError("录音已开启")
      return
    }
    ...
    try {
      ...
      if (!this.audioContext) {
        new this.OnError("浏览器不支持webAudioApi相关接口")
        return
      }
    } catch (e) {
      if (!this.audioContext) {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      navigator.mediaDevices
        .getUserMedia({
          audio: {
            echoCancellation: true, // 回声消除开关
            noiseSuppression: true, // 噪声抑制开关
            autoGainControl: true // 自动增益开关
          },
          video: false
        })
        .then((stream) => {
      ...
      if (navigator.userAgent.toLowerCase().match(/chrome/) && !location.origin.includes("https://")) {
        new this.OnError("chrome下获取浏览器录音功能，因为安全性问题，需要在localhost或127.0.0.1或https下才能获取权限")
      } else {
        new this.OnError("无法获取浏览器录音功能，请升级浏览器或使用chrome")
      }
  ...
  OnError(res) {
    console.error("WebRecorder OnError :>> ", res)
  }
}

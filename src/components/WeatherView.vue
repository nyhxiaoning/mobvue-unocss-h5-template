<template>
  <div class="weather-view">
    <van-loading
      v-if="language.globalWeatherLoading"
      ...
      <div v-if=" CupDevice?.to?.versionType === 2 ">
        ...
        <van-popup v-model:show="showPopup" position="bottom" :style="{ height: '30%' }">
          ...
          <div class="popup-content">
            <van-radio-group v-model="temperatureType">
              <van-cell-group>
                <!-- 动态渲染每个时区选项 -->
                <van-cell
                  v-for="(itemType, index) in temperatureTypes"
                  :key="index"
                  clickable
                  @click="changeTempUnitFn(itemType.value)"
                >
                  <template #title>
                    {{ itemType.label }}
                  </template>
    ...
    <div class="bottom-button">
      <van-button block type="primary" @click="confirmCity">{{
        language.confirm
      }}</van-button>
    </div>
  </div>
...
const language = reactive({
  ...
  setFahrenheit: JeeWeb.Language === "zh-CN" ? "设置华氏度℉" : "Set Fahrenheit",
  cancel: JeeWeb.Language === "zh-CN" ? "取消" : "Cancel",
  confirmError:
    JeeWeb.Language === "zh-CN"
      ? "设置失败，请重试！"
      : "Setting failed, please try again!",
  confirmSuccess: JeeWeb.Language === "zh-CN" ? "设置成功" : "Setting success!",
});
...
const changeTempUnitFn = (value) => {
  temperatureType.value = value;
  console.log(temperatureType.value, "temperatureType.value");
};
  ...
  CupDevice &&
    CupDevice.setDevMessage({
      ...
      .then((res) => {
        ...
        showToast({
          message: language.confirmSuccess,
          duration: 1000,
        });
      })
      .catch((err) => {
